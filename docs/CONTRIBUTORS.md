# Contributors

Thank you to all contributors who have helped make Bella Restaurant better!

## Core Team

### Project Lead
- **[Your Name]** - Project Owner & Lead Developer
  - GitHub: [@your-username](https://github.com/your-username)
  - Email: your.email@example.com

## Contributors

### Active Contributors

- **[Contributor Name]** - Feature development
- **[Contributor Name]** - Bug fixes and testing
- **[Contributor Name]** - Documentation

### Past Contributors

- **[Contributor Name]** - Initial setup
- **[Contributor Name]** - UI/UX improvements

## How to Contribute

We welcome contributions from the community! Here's how you can help:

1. **Report Bugs**: Open an issue with detailed information
2. **Suggest Features**: Share your ideas in discussions
3. **Submit Code**: Create a pull request with your changes
4. **Improve Docs**: Help us improve documentation
5. **Share Feedback**: Let us know what you think

## Contribution Guidelines

- Follow the [CONTRIBUTING.md](CONTRIBUTING.md) guidelines
- Read the [CODE_OF_CONDUCT.md](CONDUCT.md)
- Check existing issues before creating new ones
- Write clear commit messages
- Include tests for new features
- Update documentation as needed

## Recognition

Contributors are recognized in:
- This CONTRIBUTORS.md file
- GitHub contributors page
- Release notes
- Project README

## Getting Started

1. Fork the repository
2. Clone your fork
3. Create a feature branch
4. Make your changes
5. Submit a pull request

See [PROJECT_SETUP.md](PROJECT_SETUP.md) for detailed setup instructions.

## Questions?

- Check [CONTRIBUTING.md](CONTRIBUTING.md)
- Review [PROJECT_SETUP.md](PROJECT_SETUP.md)
- Open a discussion
- Contact the team

Thank you for contributing to Bella Restaurant!
