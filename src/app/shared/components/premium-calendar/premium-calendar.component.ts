    ...
    <div class="premium-calendar-panel" (click)="$event.stopPropagation()">
      ...
      <div class="flex justify-between items-center mb-6 sm:mb-8">
        <button type="button" (click)="prevMonth()" 
          ...
          <lucide-icon [name]="chevronLeft" class="w-5 h-5 text-white"></lucide-icon>
        </button>
        
        <div class="flex gap-2 items-center">
          <!-- Month Selection -->
          <div class="relative group">
            <button type="button" (click)="toggleCalendarSelector('month')"
                    class="font-serif italic text-xl sm:text-2xl text-white hover:text-accent transition-colors flex items-center gap-1">
              {{ currentMonthName }}
              <lucide-icon [name]="chevronDown" class="w-3 h-3 opacity-30"></lucide-icon>
            </button>
          ...
          <!-- Year Selection -->
          <div class="relative group">
            <button type="button" (click)="toggleCalendarSelector('year')"
        ...
        <button type="button" (click)="nextMonth()" 
                class="w-10 h-10 rounded-full border border-white/5 flex items-center justify-center hover:bg-accent transition-all active:scale-90">
          <lucide-icon [name]="chevronRight" class="w-5 h-5 text-white"></lucide-icon>
        </button>
      </div>

      <!-- Calendar Grid -->
      <div class="grid grid-cols-7 gap-2 sm:gap-3 text-center text-[8px] sm:text-[10px] font-bold text-white/30 uppercase tracking-[0.2em] mb-4 sm:mb-6">
